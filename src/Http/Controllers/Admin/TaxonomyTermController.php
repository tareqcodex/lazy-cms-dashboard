...
class TaxonomyTermController extends Controller
{
    public function index(Request $request, $taxonomySlug)
    {
        ...
        return view('cms-dashboard::admin.actp.taxonomies.terms', compact('taxonomy', 'terms', 'parents', 'cptSlug'));
    }
    ...
    public function ajaxStore(Request $request)
    {
        ...
        while (TaxonomyTerm::where('taxonomy_slug', $taxonomySlug)
            ->where('cpt_slug', $cptSlug)
            ->where('slug', $slug)
            ->exists()) {
            $count++;
    ...
    public function store(Request $request, $taxonomySlug)
    {
        ...
        // Ensure unique slug within the same taxonomy and CPT
        while (TaxonomyTerm::where('taxonomy_slug', $taxonomySlug)
            ->where('cpt_slug', $cptSlug)
            ->where('slug', $slug)
            ->exists()) {
