...
class PostController extends Controller
{
    ...
    protected function generateUniqueSlug($title, $id = 0, $type = 'post')
    {
        ...
        if ($postType && $postType->is_public === false) {
            return null;
        }
    ...
    public function index(Request $request)
    {
        ...
        $query = Post::with(['categories', 'tags'])->where('type', $type);
        ...
        return view('cms-dashboard::admin.posts.index', compact('posts', 'type', 'categories', 'dates', 'allCount', 'publishedCount', 'draftCount', 'trashCount', 'postType'));
    }
    ...
    public function create(Request $request)
    {
        ...
        $assignedTaxonomies = [];
        if ($type === 'post') {
            // Standard posts use built-in categories
            $assignedTaxonomies[] = (object)[
                'name' => 'Categories',
                'slug' => 'categories',
                'is_builtin' => true,
                'terms' => \Acme\CmsDashboard\Models\Category::all()
            ];
        } else {
            // Check custom taxonomies assigned to this CPT
        ...
        return view('cms-dashboard::admin.posts.create', compact('type', 'pages', 'supports', 'assignedTaxonomies', 'fieldGroups', 'postType'));
    }

    public function store(Request $request)
    {
        $this->checkTypeActive($request->input('type'));
        ...
        // Sync Built-in Categories (only for standard posts)
        if ($request->has('categories') && $request->type === 'post') {
    ...
    public function edit(Post $post)
    {
        ...
        if ($type === 'post') {
            $assignedTaxonomies[] = (object)[
                ...
                'is_builtin' => true,
                'terms' => \Acme\CmsDashboard\Models\Category::all(),
                'selected_ids' => $post->categories->pluck('id')->toArray()
            ];
        } else {
            $taxonomies = \Acme\CmsDashboard\Models\CustomTaxonomy::where('is_active', true)->get();
            foreach ($taxonomies as $tax) {
        ...
        return view('cms-dashboard::admin.posts.edit', compact('post', 'pages', 'type', 'supports', 'assignedTaxonomies', 'fieldGroups', 'fieldValues', 'postType'));
    }
    ...
    public function update(Request $request, Post $post)
    {
        ...
        if ($request->has('categories') && $post->type === 'post') {
            $post->categories()->sync($request->categories);
        }
