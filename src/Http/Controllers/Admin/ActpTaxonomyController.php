...
class ActpTaxonomyController extends Controller
{
    public function index(Request $request)
    {
        ...
        $taxonomies = $query->latest()->get();
        return view('cms-dashboard::admin.actp.taxonomies.index', compact('taxonomies'));
    }

    public function create()
    {
        $postTypes = PostType::where('is_builtin', false)->get();
        return view('cms-dashboard::admin.actp.taxonomies.create', compact('postTypes'));
    }
    ...
    public function edit($id)
    {
        ...
        $postTypes = PostType::where('is_builtin', false)->get();
        return view('cms-dashboard::admin.actp.taxonomies.edit', compact('taxonomy', 'postTypes'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        // Remove old menus before updating
        ...
        // Re-sync menus with updated post_types
    ...
    /**
     * Add sidebar sub-menu entries for this taxonomy under each assigned CPT menu.
     */
    public function syncTaxonomyMenus(CustomTaxonomy $taxonomy): void
    {
        ...
        foreach ($assignedSlugs as $cptSlug) {
            ...
            // Updated path to acpt instead of actp
            $correctRoute = '/admin/acpt/taxonomies/' . $taxonomy->slug . '/terms?cpt=' . $cptSlug;
            ...
            if (!$existing) {
                ...
                // If exists but route is wrong (due to typo actp/acpt), update it
    ...
    /**
     * Remove taxonomy sub-menu items from all CPT menus.
     */
        ...
        Menu::where('title', $taxonomy->name)
            ...
            ->where(function($q) use ($taxonomy) {
                $q->where('route', 'like', '%/acpt/taxonomies/' . $taxonomy->slug . '%')
                  ->orWhere('route', 'like', '%/actp/taxonomies/' . $taxonomy->slug . '%'); // clean up old ones too
            })
            ->delete();
    }
}
