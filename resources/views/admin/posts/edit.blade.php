<x-cms-dashboard::layouts.admin title="Edit {{ ucfirst($post->type) }}" active-menu="{{ $post->type === 'page' ? 'pages' : ($post->type ?: 'posts') }}">
    ...
    <form action="{{ route('admin.posts.update', $post) }}" method="POST" id="post-form" enctype="multipart/form-data">
        ...
        <div class="flex flex-col lg:flex-row gap-5">
            ...
            <div class="w-full lg:w-[280px] shrink-0 space-y-5">
                ...
                <!-- Standard Tags Metabox (Only for Posts) -->
                @if($post->type === 'post')
                <div class="wp-metabox mb-0">
                    <div class="wp-metabox-header flex justify-between items-center cursor-pointer">
    ...
    <script>
        ...
        document.addEventListener('click', function(e) {
            if (e.target.classList.contains('toggle-quick-add')) {
                e.preventDefault();
        ...
        document.addEventListener('click', async function(e) {
            if (e.target.classList.contains('add-term-ajax-btn')) {
                const btn = e.target;
                ...
                try {
                    let url = "{{ $post->type }}" === 'post' && (taxonomy === 'categories' || taxonomy === 'tags' || taxonomy === 'category') 
                    ...
                    if (data.id) {
                        // Add to checklist
                        const checklist = metabox.querySelector('.h-44');
                        ...
                        const inputName = (taxonomy === 'categories' || taxonomy === 'category' || taxonomy === 'tags' ) && "{{ $post->type }}" === 'post' ? 'categories[]' : 'tax_terms[]';
                        
                        label.innerHTML = `<input type="checkbox" name="${inputName}" value="${data.id}" checked class="mr-2 rounded-sm border-[#8c8f94] text-[#2271b1]"> ${data.name}`;
